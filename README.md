...
| 方法 | 路径 | 说明 |
|------|------|------|
| `POST` | `/api/notify/feishu` | 发送飞书消息通知 |
...
2. 飞书应用是否已发布并开通 `im:message:send_as_bot` 权限
3. `config/env.js` 中 `FS_BOT_WEBHOOK` 是否指向 `http://localhost:9080/api/notify/feishu`
4. 访问 `http://localhost:9080/api/notify/feishu/status` 确认配置状态
5. 如果抽奖容器使用 bridge 网络，需将 `FS_BOT_WEBHOOK` 中的 `localhost` 替换为宿主机 IP
